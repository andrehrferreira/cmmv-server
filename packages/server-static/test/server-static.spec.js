"use strict";
/**
 * @see https://github.com/expressjs/serve-static/blob/master/test/test.js
 */
...
describe('serveStatic()', function () {
    ...
    describe('fallthrough', function () {
        ...
        describe('when false', function () {
            ...
            it('should 404 when URL malformed', function (done) {
                request(this.server)
                    .get('/%')
                    .expect(404, 'sorry!', done);
            });
            it('should traversing past root', function (done) {
                request(this.server)
                    .get('/users/../../todo.txt')
                    .expect(200, done);
            });
...
function createServer(dir, opts, fn) {
    dir = dir || fixtures;
    const _serve = (0, index_1.default)(dir, opts);
    return http.createServer(function (req, res) {
        fn && fn(req, res);
        _serve.process(req, res, function (err) {
...
function shouldHaveBody(buf) {
    return function (res) {
        const body = !Buffer.isBuffer(res.body)
            ...
            : res.body;
        assert_1.strict.ok(body, 'response has body');
        assert_1.strict.strictEqual(body.toString('hex'), buf.toString('hex'));
    };
}
function shouldNotHaveBody() {
    return function (res) {
        assert_1.strict.ok(res.text === '' || res.text === undefined);
    };
}
