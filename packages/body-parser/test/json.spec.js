"use strict";
/**
 * @see https://github.com/expressjs/body-parser/blob/master/test/json.js
 */
        ...
        describe('when undefined', function () {
            ...
            it('should 400 on primitives', function (done) {
                request(this.server)
                    ...
                    .set('Content-Type', 'application/json')
                    .send('true')
                    .expect(400, '[entity.parse.failed] ' +
                    parseError('#rue').replace(/#/g, 't'), done);
            });
        });
        describe('when false', function () {
            before(function () {
...
function createServer(opts) {
    const _bodyParser = typeof opts !== 'function' ? bodyParser.jsonExpress(opts) : opts;
...
function shouldContainInBody(str) {
    return function (res) {
        assert_1.strict.ok(res.text.indexOf(str) !== -1, "expected '" + res.text + "' to contain '" + str + "'");
    };
}
